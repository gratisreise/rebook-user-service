...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| `verify-api-design` | RESTful API 설계 원칙과 엔드포인트 명명 규칙 검증 | `src/routes/**/*`, `src/controllers/**/*` |
| `verify-database` | DB 쿼리 성능, SQL 인젝션, N+1 문제 검증 | `src/models/**/*`, `src/repositories/**/*`, `migrations/**/*` |
| `verify-security` | 인증/인가, 민감 정보, 입력 검증 보안 검증 | `src/middleware/auth*`, `src/validators/**/*` |
| `verify-error-handling` | 예외 처리, 에러 응답, 로깅 일관성 검증 | `src/middleware/errorHandler*`, `src/errors/**/*` |
| `verify-performance` | 캐싱, 커넥션 풀, 비동기 처리 최적화 검증 | `src/config/cache*`, `src/config/database*` |
| `verify-testing` | 단위/통합 테스트, 커버리지 검증 | `tests/**/*`, `__tests__/**/*` |
